...
from ai.Renderer import RenderPipe
...
class RenderView(ef.Dialog):
    ...
    def __init__(self):
        ...
        def render(sender, args):
            ...
            self.set_render_image(eto_image)

        self.render_button.Click += render
    ...
    def save_image(self):
        try:
            ...
            result = save_dialog.ShowDialog(self)

            if result != ef.DialogResult.Ok:
                pass

            eto_bitmap = ed.Bitmap(self.image_view.Image)
            eto_bitmap.Save(save_dialog.FileName, ed.ImageFormat.Png)
